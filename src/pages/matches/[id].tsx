...
import { getFixtureById, getStandings } from "../../helpers/apiCalls";
...
const MatchesById = () => {
  ...
  const getRes = async () => {
    ...
    setLinear(true);
    try {
      // Use getFixtureById to fetch the match details by id
  ...
  // Only compute dateMatch if match exists
  const dateMatch = match?.fixture?.date ? new Date(match.fixture.date) : null;
  const isToday =
    dateMatch &&
